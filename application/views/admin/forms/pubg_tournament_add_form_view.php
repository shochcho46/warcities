...
<html lang="en-us">
    ...
    <body>
        ...
        <h1 class="text-center">PUBG TOURNAMENT GAME ADD FORM</h1>
        <br>
        <?php $attributes = array('id' => 'demo-form2', 'class' => 'form-horizontal form-label-left', 'name' => 'myform', 'onsubmit' => 'return  validation()'); ?>
        <?php echo form_open_multipart('admin/validation/Add_Game_Form_Validate_Controller/validatePubgAddGame', $attributes); ?>
        ...
        <div class="form-group">
            <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Select Game Type <span class="required">*</span>
            </label>
            <div class="col-md-6 col-sm-6 col-xs-12">
                <select id="gametype" name="gametype" required="required" onchange="run()">  
                    <option selected="true" disabled="disabled" value="disabled">Select</option>}  
        ...
        <script type="text/javascript">
            ...
            function validation()
            {
                ...
                var dropdown = document.getElementById("gametype").value;

                var namefunction = gameName();

                var tableNamefunction = isMObileNumber();
                var dropdownfunction = run();




                if (name === '' || sDate === '' || eDate === '' || tableName === '' || dropdown === '' || dropdown === 'disabled' || namefunction === false || tableNamefunction === false || dropdownfunction === false)
                {
                    alert(dropdown);
